...
def build_equil(lig_name, celp_st, mol, H1, H2, H3, calc_type, l1_x, l1_y, l1_z, l1_range, min_adis, max_adis, ligand_ff, ligand_ph, retain_lig_prot, ligand_charge, other_mol, solv_shell, cofactor_name, cofactor_charge):
    ...
    print('Receptor first residue in original protein file: %s' %first_res)
    ...
    # why plus 2 here? 
    p1_resid = str(int(h1_resid) - int(first_res) + 1)
    p2_resid = str(int(h2_resid) - int(first_res) + 1)
    p3_resid = str(int(h3_resid) - int(first_res) + 1)
    ...
    print(f"mol: {mol}")
    print(f"l1_x: {l1_x}, l1_y: {l1_y}, l1_z: {l1_z}")
    print(f"last rec number: {recep_resid_num}")
    print(f"l1_range: {l1_range}")
    print(f"max_adis: {max_adis}, min_adis: {min_adis}, SDRD: {sdr_dist}, other_mol_vmd: {other_mol_vmd}")
    ...
    if other_mol:
      with open(f"{cofactor_name[0]}.pdb","w+") as fh:
        for i in range(dum_atom + recep_atom + lig_atom, total_atom):
            ...
            build_file.write('%8.3f%8.3f%8.3f'%(float(coords[i][0]), float(coords[i][1]),float(coords[i][2])))
            build_file.write('%6.2f%6.2f\n'%(0, 0))
            if resname_list[i].lower() == cofactor_name[0]: 
              fh.write('%-4s  %5s %-4s %3s  %4.0f    '%('ATOM', i+1, atom_namelist[i], resname_list[i], resid_list[i]))
              fh.write('%8.3f%8.3f%8.3f'%(float(coords[i][0]), float(coords[i][1]),float(coords[i][2])))
              fh.write('%6.2f%6.2f\n'%(0, 0))
...
def create_box(comp, hmr, lig_name, mol, num_waters, water_model, ion_def, neut, buffer_x, buffer_y, buffer_z, stage, ntpr, ntwr, ntwe, ntwx, cut, gamma_ln, barostat, receptor_ff, ligand_ff, dt, dec_method, other_mol, solv_shell):
    ...
    for i in range(0, len(other_mol)):
      if other_mol[i].lower() not in ["mg","zn"]:
        tleap_vac.write('loadamberparams %s.frcmod\n'%(other_mol[i].lower()))
    ...
    #tleap_vac.write('model = combine{'+ 'receptor ' + mol.upper() + ' '+ ' '.join(m.upper() for m in other_mol if m.upper() not in ["MG","ZN"]) +'}\n\n')
    tleap_vac.write('check model\n')
    tleap_vac.write('savepdb model vac.pdb\n')
    tleap_vac.write('saveamberparm model vac.prmtop vac.inpcrd\n')
    tleap_vac.write('quit\n')
    ...
    #tleap_solvate.write('model = combine{'+ 'receptor ' + mol.upper() + ' '+ ' '.join(m.upper() for m in other_mol if m.upper() not in ["MG","ZN"]) +'}\n\n')
